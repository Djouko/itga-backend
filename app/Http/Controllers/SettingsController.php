...
{
    private const SETTING_UPDATE_FIELDS = [
        'app_name',
        'setRoomUsersLimit',
        'support_email',
        'storage_type',
        'minute_limit_in_creating_story',
        'minute_limit_in_audio_post',
        ...
        'ad_interstitial_iOS',
        'is_admob_on',
        'audio_space_hosts_limit',
        'audio_space_listeners_limit',
        'audio_space_duration_in_minutes',
    ...
    {
        if ($denied = $this->denyNonSuperAdmin($request)) {
            return $denied;
        }
    ...
    {
        $data = Cache::remember('app_settings', 300, function () {
            return Setting::first();
        });

        $interests = Cache::remember('all_interests_with_room_counts', 300, function () use ($data) {
            $interests = Interest::get();

            // Single grouped query instead of N+1 loop
            $roomLimit = (int) $data->setRoomUsersLimit;
            $blockedUserIds = User::where('is_block', 1)->pluck('id');
            $rooms = Room::whereNotIn('user_id', $blockedUserIds)
                ->where('is_private', 0)
                ->where('total_member', '<>', $roomLimit)
                ->get(['id', 'interest_ids']);

            $countByInterest = [];
            foreach ($rooms as $room) {
                $ids = explode(',', $room->interest_ids);
                foreach ($ids as $id) {
                    $id = (int) trim($id);
                    if ($id > 0) {
                        $countByInterest[$id] = ($countByInterest[$id] ?? 0) + 1;
                    }
                }
            }

            foreach ($interests as $interest) {
                $interest->totalRoomOfInterest = $countByInterest[$interest->id] ?? 0;
            }

            return $interests;
        });

        $documentType = Cache::remember('document_types', 3600, function () {
            return DocumentType::get();
        });

        $reportReasons = Cache::remember('report_reasons', 3600, function () {
            $rawReasons = ReportReason::orderBy('id', 'ASC')->get();

            return $rawReasons
                ->map(function ($reason) {
                    $reason->title = $this->normalizeReportReasonTitle((string) $reason->title);
                    return $reason;
                })
                ->filter(function ($reason) {
                    return $reason->title !== '';
                })
                ->unique(function ($reason) {
                    return $this->canonicalReportReasonKey((string) $reason->title);
                })
                ->values();
        });

        $restrictedUsernames = Cache::remember('restricted_usernames', 3600, function () {
            return UsernameRestriction::get();
        });
    ...
    public function addDocumentType(Request $request)
    {
        ...
        $documentTypeList = DocumentType::where('title', $request->title)->first();
    ...
    public function addreportReason(Request $request)
    {
        ...
        $normalizedTitle = $this->normalizeReportReasonTitle((string) $request->title);
        if ($normalizedTitle === '') {
            return response()->json([
                'status' => false,
                'message' => 'Report Reason title is required.',
            ]);
        }

        $canonicalTitle = $this->canonicalReportReasonKey($normalizedTitle);
        if ($this->reportReasonHasCanonicalColumn()) {
            $duplicateExists = ReportReason::query()
                ->where('canonical_title', $canonicalTitle)
                ->exists();
        } else {
            $duplicateExists = ReportReason::query()
                ->get(['title'])
                ->contains(function ($existingReason) use ($canonicalTitle) {
                    return $this->canonicalReportReasonKey((string) $existingReason->title) === $canonicalTitle;
                });
        }
        ...
        if ($duplicateExists) {
            ...
            $reportReason->title = $normalizedTitle;
            if ($this->reportReasonHasCanonicalColumn()) {
                $reportReason->canonical_title = $canonicalTitle;
            }
            $reportReason->save();
            $this->forgetSettingsCaches();
    ...
    public function settingPostFallback(Request $request)
    {
        foreach (self::SETTING_UPDATE_FIELDS as $field) {
        ...
        return redirect()
            ->route('setting')
            ->with('error', 'This action could not be processed. Please reload the settings page and try again.');
    }
    ...
    public function pushNotificationToSingleUser(Request $request)
    {
        ...
        $accessToken = FirebaseCredentials::getAccessToken();
        $projectId = FirebaseCredentials::getProjectId();
    ...
    private function normalizeReportReasonTitle(?string $title): string
    {
        $sanitizedTitle = InputSanitizer::sanitizeText($title, 120);
        if ($sanitizedTitle === null) {
            return '';
        }

        return preg_replace('/\s+/u', ' ', trim($sanitizedTitle)) ?? '';
    }

    private function canonicalReportReasonKey(string $title): string
    {
        return mb_strtolower($this->normalizeReportReasonTitle($title), 'UTF-8');
    }

    private function reportReasonHasCanonicalColumn(): bool
    {
        static $hasCanonicalColumn = null;

        if ($hasCanonicalColumn !== null) {
            return $hasCanonicalColumn;
        }

        $hasCanonicalColumn = Schema::hasTable('report_reasons')
            && Schema::hasColumn('report_reasons', 'canonical_title');

        return $hasCanonicalColumn;
    }

    private function forgetSettingsCaches()
    {
        Cache::forget('app_settings');
        Cache::forget('all_interests_with_room_counts');
        Cache::forget('document_types');
        Cache::forget('report_reasons');
        Cache::forget('restricted_usernames');
    }

    private function denyNonSuperAdmin(Request $request)
    {
