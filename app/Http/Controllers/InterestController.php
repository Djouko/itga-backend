...
class InterestController extends Controller
{
   ...
   {
    $data = cache()->remember('interests_with_room_counts', 600, function () {
        $fetchInterests = Interest::all();

        $roomCounts = Room::selectRaw('interest_ids, COUNT(*) as cnt')
            ->groupBy('interest_ids')
            ->pluck('cnt', 'interest_ids');

        foreach ($fetchInterests as $fetchInterest) {
            $fetchInterest->totalRoomOfInterest = $roomCounts[$fetchInterest->id] ?? 0;
        }
