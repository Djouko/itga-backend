$(document).ready(function () {
  ...
  if (eye1 && eyeoff1 && passwordField1) {
  ...
  $(document).on("submit", "#deepLinkingForm", function (e) {
    e.preventDefault();
    if (user_type == 1) {
      let formData = new FormData($("#deepLinkingForm")[0]);
      $.ajax({
        type: "POST",
        ...
        success: function (response) {
          if (response.status == true) {
            iziToast.show({
              title: "Success",
              message: "Deep Linking save successfully.",
              color: "green",
  ...
  $(document).on("submit", "#androidDeepLinkingForm", function (e) {
    e.preventDefault();
    if (user_type == 1) {
      ...
      formData.set("sha_256", shaValues.join(","));

      $.ajax({
        type: "POST",
        url: `${domainUrl}androidDeepLinking`,
        data: formData,
        contentType: false,
        processData: false,
        success: function (response) {
          if (response.status == true) {
            iziToast.show({
              title: "Success",
              message: "Android Deep Linking save successfully.",
              color: "green",
              position: toastPosition,
              transitionIn: "fadeInUp",
              transitionOut: "fadeOutDown",
  ...
  $(document).on("submit", "#iOSDeepLinkingForm", function (e) {
    e.preventDefault();
    if (user_type == 1) {
      let formData = new FormData($("#iOSDeepLinkingForm")[0]);
      $.ajax({
        type: "POST",
        url: `${domainUrl}iOSDeepLinking`,
        data: formData,
        contentType: false,
        processData: false,
        success: function (response) {
  ...
  $(document).on("submit", "#reportReasonForm", function (e) {
    e.preventDefault();
    if (user_type == 1) {
      const reportReasonInput = $("#reportReason");
      const normalizedTitle = ((reportReasonInput.val() || "") + "")
        .trim()
        .replace(/\s+/g, " ");
      ...
      if (!normalizedTitle) {
        iziToast.show({
          title: "Invalid Title",
          message: "Please enter a valid report reason.",
          color: "red",
          position: toastPosition,
          transitionIn: "fadeInUp",
          transitionOut: "fadeOutDown",
      ...
      reportReasonInput.val(normalizedTitle);
      ...
      $.ajax({
        ...
        success: function (response) {
          if (response.status == false) {
            iziToast.show({
              title: "Reason Not Saved",
              message: response.message || "Report reason already exists.",
              color: "red",
              position: toastPosition,
              transitionIn: "fadeInUp",
              transitionOut: "fadeOutDown",
  ...
  $(document).on("submit", "#editReportReasonForm", function (e) {
    ...
    if (user_type == 1) {
      ...
      const editReasonInput = $("#editReportReason");
      const normalizedTitle = ((editReasonInput.val() || "") + "")
        .trim()
        .replace(/\s+/g, " ");
      ...
      editReasonInput.val(normalizedTitle);
