...
function getAdminCommentActor(comment) {
  if (comment.admin_actor) {
    return comment.admin_actor;
  }

  return {
    id: comment.user?.id || 0,
    username: comment.user?.username || "",
    profile: comment.user?.profile || "asset/image/default.png",
    type: "user",
  };
}

function getAdminCommentActorBadge(actor) {
  return actor.type === "company"
    ? '<span class="badge bg-info ms-2">Entreprise</span>'
    : "";
}
      ...
      success: function (response) {
        if (response.status) {
          ...
          postComments.forEach((comment) => {
            const actor = getAdminCommentActor(comment);
            const actorBadge = getAdminCommentActorBadge(actor);
            const commentHtml = `
              ...
              actor.id
                    ...
                    <div class="d-flex align-items-center">
                      <div class="post-modal-user-profile mr-2">
                        <img src="${
                          actor.profile || "asset/image/default.png"
                        }" alt="" id="user-profile">
                      </div>
